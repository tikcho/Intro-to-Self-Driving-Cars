...
vector< vector<float> > normalize(vector< vector <float> > grid) {
    ...
    for (int row2 = 0; row2<grid.size(); row2++){
        newRow.clear();
        for (int cell2 = 0; cell2<grid[row2].size(); cell2++){
            newRow.push_back(grid[row2][cell2]/total);
        }
...
// 	vector < vector <float> > newGrid;
    ...
    float cenp = 1.0 - blurring;
    float corp = blurring / 12.0;
    float adjp = blurring / 6.0;
    
    vector < vector <float> > window {{corp, adjp, corp}, {adjp, cenp, adjp}, {corp, adjp, corp}};
