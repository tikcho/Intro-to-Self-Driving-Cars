...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    vector<float>::size_type area = height * width;
	...
	float p_miss) 
{
    ...
    for (int i=0; i<beliefs.size(); i++){
        for (int j=0; j<beliefs[0].size(); j++){
            if (color == grid[i][j]){ hit = 1; }
            else { hit = 0; }
            
            s0 = beliefs[i][j]* (hit * p_hit + (1-hit) * p_miss);
            newRow.push_back(s0);
            s += s0;
        }
    ...
    for (int i2=0; i2<newGrid.size(); i2++){
        for (int j2=0; j2<newGrid[0].size(); j2++){
            newGrid[i2][j2] = newGrid[i2][j2] / s;
        }
    }
